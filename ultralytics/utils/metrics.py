...
def bbox_iou(box1, box2, xywh=True,
             GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False, MPDIoU=False,
             Focal=False, gamma=0.5, alpha=1, scale=False, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    ...
    Args:
        ...
        SIoU (bool, optional): If True, calculate SIoU. Defaults to False.

        EIoU (bool, optional): If True, calculate EIoU. Defaults to False.
        Focal: Focal_EIoU中的Focal。
               为True时，可结合其他的?IoU生成对应的IoU变体，如 Focal_CIoU 设置为 CIoU=True, Focal=True
               测试的过程中，除了Focal_SIoU出现loss为inf之外，其他的都正常，不过这个不同的数据集可能出现不一样，具体可以自行测试下
        gamma: Focal_?IoU中的gamma参数，默认为0.5

        alpha: AlphaIoU中的alpha参数。
               alpha为1时: 普通的IoU, alpha为3时: AlphaIoU (论文默认值)
               AlphaIoU的思想同样可以用在其他的IoU变种上:
               如果设置alpha为3，其他IoU设定的参数(GIoU，DIoU，CIoU，EIoU，SIoU)为False的时候，那就是AlphaIoU
               如果设置alpha为3，CIoU为True的时候，那就是Alpha_CIoU，效果的话就因数据集和模型而异，具体可以自行测试下

        WIoU (bool, optional): If True, calculate WIoU. Defaults to False.
        scale: WIoU使用，scale为True时，会乘以一个系数
        monotonous: 位于WIoU函数中，3个输入分别代表WIoU的3个版本
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU or MPDIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU or MPDIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                elif scale:  # WIoU https://arxiv.org/abs/2301.10051
            ...
            elif MPDIoU:  # MPDIoU https://arxiv.org/pdf/2307.07662.pdf
                if Focal:
                    raise RuntimeError("MPDIou do not support Focal. (Maybe)")
                cw2 = torch.pow(cw ** 2 + eps, alpha)
                ch2 = torch.pow(ch ** 2 + eps, alpha)
        ...
        c_area = cw * ch + eps  # convex area
        if Focal:  # Focal_GIoU https://arxiv.org/pdf/1902.09630.pdf
...
class WIoU_Scale:
    ...
    Attributes:
        monotonous: 需要设置scale，scale为True时，WIoU会乘以一个系数，结合monotonous即会对应WIoU的3个版本。
            values: {
                None: origin              v1
        ...
        _momentum: The momentum of running mean
    """
